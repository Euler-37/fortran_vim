...
syn keyword fortranIO		access blank direct exist file fmt formatted iostat name named nextrec number opened rec recl sequential status unformatted unit
...
syn keyword fortranIntrinsic	co_max co_min co_sum co_reduce co_broadcast
...
syn match  fortranConditional	"\<change\s*team"
syn match  fortranConditional	"\<end\s*team"
syn match  fortranUnitHeader	"\<sync\s*all\>"
syn match  fortranUnitHeader	"\<form\s*team\>"
...
if b:fortran_dialect == "f08"
  ...
  syn keyword fortranType             c_ptr c_funptr team_type
  " ISO_Fortran_env
...
hi def link fortranCall		fortranPreCondit

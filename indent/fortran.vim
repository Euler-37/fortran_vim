...
function FortranGetIndent(lnum)
  ...
  if prevstat =~? '^\s*\(case\|type\s*is\|class\s*is\|class\s*default\|else\|else\s*if\|else\s*where\)\>'
	\ ||prevstat=~? '^\s*\(type\|interface\|abstract\s*interface\|associate\|enum\)\>'
  ...
  " module function\subroutine\procedure
  if !exists("b:fortran_indent_less") && !exists("g:fortran_indent_less")
    ...
    if prevstat =~? '^\s*\(contains\|submodule\|program\)\>'
            \ ||prevstat =~? '^\s*module\>'
            \ ||prevstat =~? '^\s*'.prefix.'\(module\)*\s*subroutine\>'
            \ ||prevstat =~? '^\s*'.prefix.type.'\(module\)*\s*function\>'
            \ ||prevstat =~? '^\s*'.type.prefix.'\(module\)*\s*function\>'
      let ind = ind + shiftwidth()
    endif
    " overload interface
    if prevstat =~? '^\s*\(module\s*procedure\)\>' 
          \&& getline(v:lnum)=~? '^\s*\(module\s*procedure\)\>' 
      let ind = ind - shiftwidth()
    endif
    " end of overload interface
    if prevstat =~? '^\s*\(module\s*procedure\)\>' 
          \&& getline(v:lnum)=~? '^\s*\(end\s*interface\)\>' 
      let ind = ind - shiftwidth()
    endif
    " end procedure
    if getline(v:lnum) =~? '^\s*contains\>'
          \ ||getline(v:lnum)=~? '^\s*end\s*'
          \ .'\(function\|subroutine\|module\|submodule\|program\|procedure\)\>'
      let ind = ind - shiftwidth()
    endif
  endif
  ...
  if getline(v:lnum) =~? '^\s*\(\d\+\s\)\=\s*'
        \. '\(else\|else\s*if\|else\s*where\|case\|type\s*is\|class\s*is\|class\s*default\|'
